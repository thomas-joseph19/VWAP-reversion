...
- [ ] Compute Daily VWAP from trade prints (price x size weighting)
- [ ] Compute Weekly VWAP (rolling across sessions within the week)
- [ ] Compute Monthly VWAP (rolling across sessions within the month)
- [ ] Compute VWAP standard deviation bands in real-time (1s through 4s)
- [ ] Build daily volume profiles from trade prints (price x volume histogram)
...
- [ ] Detect when price is meaningfully extended from VWAP (1.7s to 3s range)
...
- Median max deviation before return: ~2.0s
- 75th percentile: ~2.6s, 90th percentile: ~3.0s
- Return probability: 1.0-1.5s -> ~96%, 1.5-2.0s -> ~93-95%, 2.0-2.5s -> ~88-92%
- Highest probability zone: 1.7s to 2.2s
...
- **Performance**: Must handle ~884 files x ~150K rows each efficiently; consider chunked processing or pre-filtering
...
| Mid-price for VWAP when no trades | Synthetic inputs would bias the indicator and break causal validation | Do not impute mid-price volume; forward-fill the last causal trade-derived value within each session |
